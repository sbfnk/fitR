...
SE2IR_linDecay_deter_name <- "deterministic SEIR model with constant population size, erlang distribution for E (shape=2) and linear decay of reproduction number"
SE2IR_linDecay_state.names <- c("S","E1", "E2", "I","R","Inc")
SE2IR_linDecay_theta.names <- c("R0", "D_lat", "D_inf", "rho", "t_intervention", "alpha")

calculate_R_t <- function(time, R0, t_intervention, alpha) {

	R_t <- rep(R0, len = length(time))
	
	time_after_intervention <- time[time >= t_intervention] - t_intervention

	R_t[time >= t_intervention] <- R_t[time >= t_intervention] - alpha * (time_after_intervention)
...
# Solves the system of ordinary differential equations for the SE2IR_linDecay model.
SE2IR_linDecay_simulateDeterministic <- function(theta,init.state,times) {

	SE2IR_linDecay_ode <- function(time, state, theta) {
...
SE2IR_linDecay_genObsPoint <- function(model.point, theta){
...
SE2IR_linDecay_prior <- function(theta, log = FALSE) {
	...
	log.prior.linear.decay <- dunif(theta[["alpha"]], min = 0, max = 1, log = TRUE)
	log.prior.reporting.rate <- dunif(theta[["rho"]], min = 0, max = 1, log = TRUE)

	log.sum = log.prior.R0 + log.prior.latent.period + log.prior.infectious.period + log.prior.t_intervention + log.prior.linear.decay + log.prior.reporting.rate
...
SE2IR_linDecay_pointLike <- function(data.point, model.point, theta, log = FALSE){
...
# create fitmodel
SE2IR_linDecay_deter <- fitmodel(
	name=SE2IR_linDecay_deter_name,
	state.names=SE2IR_linDecay_state.names,
	theta.names=SE2IR_linDecay_theta.names,
	simulate=SE2IR_linDecay_simulateDeterministic,
	dprior=SE2IR_linDecay_prior,
	rPointObs=SE2IR_linDecay_genObsPoint,
	dPointObs=SE2IR_linDecay_pointLike)
