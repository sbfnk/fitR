...
plotTraj <- function(traj,state.variables=NULL,alpha=1, plot=TRUE) {

    if(is.null(state.variables)){
        state.variables <- colnames(traj)
        ## remove time column from state variables
        state.variables <- state.variables[!(state.variables == "time")]
    }
