...
#'
#'A \code{fitmodel} object is a \code{list} that stores some variables and functions that will be useful to simulateTraj and fit your model during the course. 
...
#' @param simulateTraj \R-function to simulateTraj forward the model (required). This function takes 3 arguments:
#' \itemize{
#' \item \code{theta} named vector of model parameters.
#' \item \code{state.init} named vector of initial state of the model.
#' \item \code{times} time sequence for which state of the model is wanted; the first value of times must be the initial time, i.e. the time of \code{initial.state}.
#' }
...
#' @param generateObservation \R-function that generate simulated data from a simulated trajectory using an observation model. This function takes 2 arguments:
#' \itemize{
#' \item \code{simu.traj} data.frame of simulated trajectories, as returned by \code{simulateTraj}.
#' \item \code{theta} named vector of model parameters.
#' }
#' and return the \code{simu.traj} data.frame with an additional "observation" column generated by the observation model. (optional)
#' @param logPrior \R-function to evaluate the log-prior density of the parameters at a given \code{theta}. (optional) The function should take 1 argument:
#'#' \itemize{
#' 	\item \code{theta} named vector of model parameter values.
#' }
#' and returns the logged value of the prior density distribution.
#' @param logLikePoint \R-function to evaluate the log-likelihood of one data point given the state of the model at the same time point. This function takes 3 arguments:
#' \itemize{
...
#' \item \code{theta} named vector of model parameters. Useful if some parameters are needed to compute the likelihood (i.e. reporting rate).
#' }
