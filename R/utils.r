...
#' @param observation logical, if \code{TRUE} simulated obseravtion are generated by \code{fitmodel$generateObs}.
...
simulateModelReplicates <- function(fitmodel,theta, state.init, times, n, observation=FALSE) {
    ...
    if(observation && is.null(fitmodel$generateObs)){
        stop("Can't generate observation as ",sQuote("fitmodel")," doesn't have a ",sQuote("generateObs")," function.")
    }
