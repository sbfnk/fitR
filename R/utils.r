...
#' @param observation logical, if \code{TRUE} simulated observation are generated by \code{rObsTraj}.
...
simulateFinalStateAtExtinction <- function(fitmodel, theta, init.state, extinct=NULL ,time.init=0, time.step=100, n=100, observation=FALSE, n.cores = 1) {
    ...
    final.state.rep <- ldply(rep,function(x) {
        ...
        while(any(current.state[extinct]>=0.5)){
            ...
            if(observation){
                traj <- rObsTraj(fitmodel, theta, current.state, times)
            } else {
...
#' @param trace either a \code{data.frame} or a \code{list} of \code{data.frame} with all variables in column, as outputed by \code{\link{rmcmcMH}}. Accept also an \code{mcmc} or \code{mcmc.list} object.
